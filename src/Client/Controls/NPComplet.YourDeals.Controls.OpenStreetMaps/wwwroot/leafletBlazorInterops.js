...
window.leafletBlazor = {
    create: function (map, objectReference) {
        ...
        var leafletMap = L.map(map.id,
            {
                ...
                maxBounds: map.maxBounds && map.maxBounds.item1 && map.maxBounds.item2
                    ? L.latLngBounds(map.maxBounds.item1, map.maxBounds.item2)
                    : undefined,
            });
      
        connectMapEvents(leafletMap, objectReference);
        maps[map.id] = leafletMap;
        var corners = leafletMap._controlCorners,
            l = 'leaflet-',
        ...
        leafletMap.zoomControl.setPosition('verticalcenterright');
       
    },
    setZoom: function(mapId, zoom, latLng) {
        var map = maps[mapId];
        ...
        const layer = L.tileLayer(tileLayer.urlTemplate,
            {
                ...
                bounds: tileLayer.bounds && tileLayer.bounds.item1 && tileLayer.bounds.item2
                    ? L.latLngBounds(tileLayer.bounds.item1, tileLayer.bounds.item2)
                    : undefined,
                // ---
    ...
    addRectangle: function(mapId, rectangle, objectReference) {
        const layer =
            L.rectangle([[rectangle.shape.bottom, rectangle.shape.left], [rectangle.shape.top, rectangle.shape.right]],
                createPolyline(rectangle));
    ...
    updateRectangle: function(mapId, rectangle) {
        ...
        if (layer !== undefined) {
            layer.setBounds([
                [rectangle.shape.bottom, rectangle.shape.left], [rectangle.shape.top, rectangle.shape.right]
            ]);
        }
    },
...
function createIcon(icon) {
    return L.icon({
        ...
        shadowSizeAnchor: icon.shadowSizeAnchor
            ? L.point(icon.shadowSizeAnchor.value.width, icon.shadowSizeAnchor.value.height)
            : null,
    ...
    layerObj.bindPopup(popup.content,
        {
            ...
            autoPanPaddingTopLeft: popup.autoPanPaddingTopLeft
                ? L.point(popup.autoPanPaddingTopLeft.x, popup.autoPanPaddingTopLeft.y)
                : null,
            autoPanPaddingBottomRight: popup.autoPanPaddingBottomRight
                ? L.point(popup.autoPanPaddingBottomRight.x, popup.autoPanPaddingBottomRight.y)
                : null,
