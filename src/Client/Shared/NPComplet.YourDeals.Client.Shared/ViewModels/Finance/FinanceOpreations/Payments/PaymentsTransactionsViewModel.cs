...
using NPComplet.YourDeals.Client.Shared.WebUI.Pages.Finance.FinanceOpreations.Payments;

namespace NPComplet.YourDeals.Client.Shared.ViewModels.Finance.FinanceOpreations.Payments
{
    public class PaymentsTransactionsViewModel : ViewModelBase
    {
        ...
        public IList<CrossingPayment> CrossPayments { get; set; } = new List<CrossingPayment>();

        public PaymentsTransactionsViewModel(HttpClient httpClient, NotificationService notificationService, AntDesign.DrawerService drawerService)
        {
        ...
        public async Task GetTransactionsAsync(int pageindex,int size)
        {
          
            var ListPaymentsResponse = await _httpClient.GetFromJsonAsync<Result<IEnumerable<CrossingPayment>>>(string.Format(Config.GetAllUserTransaction,pageindex,size));
            ...
            if (ListPaymentsResponse.Succeeded)
            {
                ...
                {
                    CrossPayments = new List<CrossingPayment>();
                    foreach (var crossingPayment in ListPaymentsResponse.Data)
                    {

                        CrossPayments.Add(crossingPayment);
                    };
                }
            }
        }
        ...
        public override async Task OnInitializedAsync()
        {
            ...
            await GetTransactionsAsync(0, PageSize);
            StateHasChanged();
            Loading = false;
        ...
        {
            Loading = true;
            await GetTransactionsAsync((int)(args.Skip != null ? args.Skip : 0), args.Top.Value);
