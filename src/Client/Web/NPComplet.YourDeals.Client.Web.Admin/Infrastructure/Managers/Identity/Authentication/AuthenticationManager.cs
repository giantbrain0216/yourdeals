...
using NPComplet.YourDeals.Client.Web.Admin.Infrastructure.Routes;
...
namespace NPComplet.YourDeals.Client.Web.Admin.Infrastructure.Managers.Identity.Authentication
{
    public class AuthenticationManager : IAuthenticationManager
    {
        ...
        public async Task<IResult> Login(LoginViewModel model)
        {
            ...
            var result = await response.ToResult<AuthResponseDto>();
            if (result.Succeeded)
            {
              ...
              //  var refreshToken = result.Data.RefreshToken;
               // var userImageURL = result.Data.UserImageURL;
                ...
                //}
                ((NPCompletStateProvider)this._authenticationStateProvider).MarkUserAsAuthenticated(model.Email);
        ...
        public async Task<IResult> Logout()
        {
            ...
            ((NPCompletStateProvider)_authenticationStateProvider).MarkUserAsLoggedOut();
        ...
        public async Task<string> RefreshToken()
        {
            ...
            var refreshToken = await _localStorage.GetItemAsync<string>(StorageConstants.Local.RefreshToken);
             ...
             var response = await _httpClient.PostAsJsonAsync(Routes.TokenEndpoints.Refresh, new AuthResponseDto { Token = token, RefreshToken = refreshToken });
           // var response = await _httpClient.PostAsJsonAsync(Routes.TokenEndpoints.Refresh,  token);
