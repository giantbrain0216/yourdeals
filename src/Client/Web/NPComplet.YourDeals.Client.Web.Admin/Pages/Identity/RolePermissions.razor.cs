...
{
    public partial class RolePermissions : ComponentBase
    {
        ...
        private RoleClaimViewModel _selectedItem = new();
        ...
        private async Task SaveAsync()
        {
            ...
            var result = await RoleManager.UpdatePermissionsAsync(_model);
            if (result.Succeeded)
            {
                _snackBar.Add(result.Messages[0], Severity.Success);
                await HubConnection.SendAsync(ApplicationConstants.SignalR.SendRegenerateTokens);
                await HubConnection.SendAsync(ApplicationConstants.SignalR.OnChangeRolePermissions, _currentUser.GetUserId(), _model.RoleId);
                _navigationManager.NavigateTo("/identity/roles");
            }
            else
            {
