...
using NPComplet.YourDeals.Client.Web.Admin.Infrastructure.Settings;

namespace NPComplet.YourDeals.Client.Web.Admin.Shared
{
           ...
           // _rightToLeft = await _clientPreferenceManager.IsRTL();
            ...
            hubConnection.On<string, string>(ApplicationConstants.SignalR.LogoutUsersByRole, async (userId, roleId) =>
            {
                if (CurrentUserId != userId)
                {
                    ...
                    {
                        var role = rolesResponse.Data.FirstOrDefault(x => x.Id.ToString() == roleId);
                        if (role != null)
                        {
